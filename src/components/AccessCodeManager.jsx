    ...
    const handleCreateAccount = async (e) => {
        ...
        try {
            ...
            try {
                ...
                // Check for email-already-in-use error (handle both code and message for robustness)
                if (authError.code === 'auth/email-already-in-use' || authError.message.includes('email-already-in-use')) {
                    console.log("Account exists. Overwriting/Reactivating...");

                    // Account exists in Auth. We must overwrite the Firestore doc to ensure a clean state.
                    // We use setDoc WITHOUT merge to completely replace the old data (preventing old mess/role from persisting).
                    await setDoc(doc(db, "users", email), {
                        ...
                        deleted: false, // Ensure it's active
                        reactivatedAt: new Date().toISOString() // Audit trail
                    });

                    alert(`Account "${newCode}" already existed. It has been completely reset and reactivated with the new details. You will now be logged out.`);
